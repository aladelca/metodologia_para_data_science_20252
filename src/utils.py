...
def crear_promedios_moviles(serie, ventanas=None, prefijo='ma'):
    ...
    for ventana in ventanas:
        df[f'{prefijo}_{ventana}'] = serie.shift(1).rolling(window=ventana, min_periods=1).mean()
...
def crear_std_moviles(serie, ventanas=None, prefijo='std'):
    ...
    df = pd.DataFrame(index=serie.index)
    for ventana in ventanas:
        df[f'{prefijo}_{ventana}'] = serie.shift(1).rolling(window=ventana, min_periods=1).std()
...
def calcular_velocidad_momentum_corregida(serie, ventanas=None, metodo='lineal', columna_target='target'):
    ...
    for ventana in ventanas:
        if metodo == 'lineal':
            ...
            df[f'velocidad_log_{ventana}'] = (np.log(serie) - np.log(serie.shift(ventana))) / ventana
        elif metodo == 'pct':
            df[f'velocidad_pct_{ventana}'] = serie.pct_change(ventana) / ventana
...
def crear_features_completas(serie, config=None):
    ...
    if 'ventanas_roll' in config:
        df_roll = crear_variables_rolling_avanzadas(serie, config['ventanas_roll'])
        df_final = pd.concat([df_final, df_roll], axis=1)

    # Technical indicators
    if config.get('incluir_tecnicas', True):
        df_tech = crear_variables_tecnicas(serie, config.get('ventanas_cortas'), config.get('ventanas_largas'))
    ...
    if config.get('incluir_fechas', True):
        ...
        df_final = pd.concat([df_final, df_fechas_basicas, df_fechas_especiales,
                             df_fechas_estacionales, df_fechas_mercado], axis=1)

    # Momentum indicators
    if config.get('incluir_momentum', True):
        df_momentum = calcular_momentum_simple(serie, config.get('ventanas_momentum'))
        df_velocidad = calcular_velocidad_momentum_corregida(serie, config.get('ventanas_momentum'))
...
def verificar_no_data_leakage(features, target_col='target', mostrar_ejemplo=True):
    ...
    correlaciones_futuras.sort(key=lambda x: abs(x[1]), reverse=True)

    if mostrar_ejemplo and correlaciones_futuras:
        print(f"Top 5 correlaciones con valores futuros:")
...
def auto_arima_manual(serie, max_p=30, max_d=2, max_q=30, criterio='aic'):
    ...
    print(f"Best {criterio.upper()}: {best_aic if criterio == 'aic' else best_bic}")
