...
def test_predict_single_model(model_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            forecast_period = (
                f"{data['forecast_start_date']} to "
                f"{data['forecast_end_date']}"
            )
            print(f"Forecast Period: {forecast_period}")
            print(f"Total Models Used: {data['total_models_used']}")
            prediction_time = data['total_prediction_time_seconds']
            ...
            if data['predictions']:
                ...
                pred_first_3 = pred['predictions'][:3]
                print(f"  Predictions (first 3): {pred_first_3}")
                print(f"  Dates (first 3): {pred['dates'][:3]}")
                if pred['confidence_lower']:
                    print("  Has confidence intervals: Yes")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for pred in data['predictions']:
                model_info = f"{pred['model_name']} ({pred['model_type']})"
                print(f" - {model_info}")

            if data['ensemble_prediction']:
                ensemble_first_3 = data['ensemble_prediction'][:3]
                print(f"\nEnsemble Prediction (first 3): {ensemble_first_3}")
...
def main():
    ...
    if models:
        ...
        test_name = f"Predict ({first_model})"
        results.append((test_name, test_predict_single_model(first_model)))
